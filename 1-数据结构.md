...
## 均摊复杂度线段树（Segment Tree Beats）

+ 区间取 min，区间求和。

```cpp
// ll = long long, N is a constant
struct Node{
    ll mx,smx,sum;
    int cnt;
...
#define lson (t[now].ls)
#define rson (t[now].rs)
    struct Node{int ls,rs,val,siz;ui pri;}t[N];
    int rt,tot;

    void pushup(int now) {t[now].siz=t[lson].siz+t[rson].siz+1;}
    int merge(int now,int now1){
        if(!now||!now1) return now|now1;
        if(t[now].pri<t[now1].pri){ 
            t[now1].ls=merge(now,t[now1].ls);
            pushup(now1);return now1;
        }
        else{
            t[now].rs=merge(t[now].rs,now1);
    ...
    void split(int now,int val,int &rt1,int &rt2){
        ...
        if(val<t[now].val){
            split(lson,val,rt1,rt2);
            t[now].ls=rt2,rt2=now;
        }
        else{
            split(rson,val,rt1,rt2);
            t[now].rs=rt1,rt1=now;
        }
        pushup(now);
    }
    void split_by_siz(int now,int siz,int &rt1,int &rt2){
        if(!now) {rt1=rt2=0;return;}
        if(siz<=t[lson].siz){
            split_by_siz(lson,siz,rt1,rt2);
            t[now].ls=rt2,rt2=now;
        }
        else{
            split_by_siz(rson,siz-t[lson].siz-1,rt1,rt2);
            t[now].rs=rt1,rt1=now;
        }
        pushup(now);
    }
    int _rank(int now,int val){
        if(!now) return 1;
        if(val<=t[now].val) return _rank(lson,val);
        else return t[lson].siz+1+_rank(rson,val);
    }
    int _kth(int now,int k){
        if(k<=t[lson].siz) return _kth(lson,k);
        else if(k==t[lson].siz+1) return t[now].val;
        else return _kth(rson,k-t[lson].siz-1);
    }
    ...
    void insert(int val){
        int now=newnode(val);
        int p1,p2;split(rt,val,p1,p2);
        rt=merge(p1=merge(p1,now),p2);
    }
    void erase(int val){
        int p1,p2,p3,p4;
        split(rt,val,p1,p2);
        split_by_siz(p1,t[p1].siz-1,p3,p4);
        rt=merge(p3,p2);
    }
    inline int rank(int val) {return _rank(rt,val);}
    inline int kth(int k) {return _kth(rt,k);}
    inline int prev(int val) {return kth(rank(val)-1);}
    inline int next(int val) {return kth(rank(val+1));}
#undef lson
#undef rson
}t;
```
...
template <typename T>
T rand(T l,T r) {return uniform_int_distribution<>(l,r)(rng);}
...
#define rson (t[now].rs)
    ...
    void pushup(int now){
        t[now].siz=t[lson].siz+t[rson].siz+1;
        t[now].sum=t[lson].sum+t[rson].sum+t[now].val;
    }
    int newnode(int v) {t[++tot]={0,0,1,v,v,0};return tot;}
    int cp(int now) {t[++tot]=t[now];return tot;}
    void rev(int &now){
        if(!now) return;
        now=cp(now);
        t[now].rev^=1,swap(lson,rson);
    }
    void pushdown(int &now){
        if(!now) return;
        now=cp(now);
        if(t[now].rev) 
            rev(lson),rev(rson),t[now].rev=0;
    }
    int merge(int nw1,int nw2){
        if(!nw1||!nw2) return nw1|nw2;
        pushdown(nw1),pushdown(nw2);
        if(rand(1,t[nw1].siz+t[nw2].siz)<=t[nw1].siz){
            t[nw1].rs=merge(t[nw1].rs,nw2);
            pushup(nw1);return nw1;
        }
        else{
            t[nw2].ls=merge(nw1,t[nw2].ls);
            pushup(nw2);return nw2;
        }
    }
    void split(int now,int k,int &rt1,int &rt2){
        if(!now) {rt1=rt2=0;return;}
        pushdown(now);
        if(t[lson].siz<k){
            split(rson,k-t[lson].siz-1,rt1,rt2);
            rson=rt1,rt1=now;pushup(now);
        }
        else{
            split(lson,k,rt1,rt2);
            lson=rt2,rt2=now;pushup(now);
        }
    }
#undef lson
#undef rson
...
int insert(int rt,int k,int x){
    int r1,r2;t.split(rt,k,r1,r2);
    return t.merge(t.merge(r1,t.newnode(x)),r2);
}

int erase(int rt,int x){
    int r1,r2,r3,tmp;
    t.split(rt,x-1,r1,r2);
    t.split(r2,x,r2,r3);
    if(r2) t.split(r2,1,tmp,r2);
    return t.merge(t.merge(r1,r2),r3);
}

int rev(int rt,int l,int r){
    int r1,r2,r3;
    t.split(rt,l-1,r1,r2);
    t.split(r2,r-l+1,r2,r3);
    t.rev(r2);
    return t.merge(r1,t.merge(r2,r3));
}
```
